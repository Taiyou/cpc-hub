# CPC Hub — Claude Code PoC 設計書 (v2)

> **このドキュメントの目的**: Claude Code（CLI）にこのファイルを渡して、CPC Hub プロジェクトの PoC を自律的に実装させるための完全な設計書。本書をリポジトリ root に `DESIGN.md` として配置し、`claude` コマンドから参照させる。
...
#### テンプレート構造（日英両言語）
```markdown
# CPC Hub Newsletter — {month_name} {year}
...
```
依頼例: "CPC Hub の Phase 1 を実装して。DESIGN.md の §3 と §5 を参照。
...
```markdown
# Claude Code Conventions for CPC Hub
